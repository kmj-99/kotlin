...
object FirJsNameClashClassMembersChecker : FirClassChecker() {
    private class StableNamesCollector {
        ...
        private fun FirTypeScope.collectOverriddenLeaves(classMemberSymbol: FirCallableSymbol<*>): Set<FirCallableSymbol<*>> {
            val processedSymbols = hashSetOf(classMemberSymbol)
            val toProcessSymbols = mutableListOf(classMemberSymbol)
    ...
    override fun check(declaration: FirClass, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        for ((name, stableNames) in membersGroupedByName.entries) {
            val fakeOverrideStableNames = stableNames.filterTo(mutableSetOf()) {
                it.symbol.getContainingClassSymbol(context.session) != declaration.symbol
            }
            for (stableName in stableNames) {
                var clashed = stableName.clashedNamesWithThis(stableNames)
                if (stableName in fakeOverrideStableNames) {
                    // Do not check for clashes between fake overrides here.
                    // Such clashes will be checked with JS_FAKE_NAME_CLASH below.
                    clashed = clashed.filter { it !in fakeOverrideStableNames }
                }
                if (clashed.isNotEmpty()) {
                    val source = stableName.symbol.source ?: declaration.source
                    reporter.reportOn(source, FirJsErrors.JS_NAME_CLASH, name, clashed.map { it.symbol }, context)
                }
            }
            for (fakeOverrideStableName in fakeOverrideStableNames) {
                val intersectedSymbols = stableNameCollector.overrideIntersections[fakeOverrideStableName.symbol] ?: emptySet()
                val clashed = fakeOverrideStableName.clashedNamesWithThis(fakeOverrideStableNames).filter {
                ...
                reporter.reportOn(
                    declaration.source,
                    FirJsErrors.JS_FAKE_NAME_CLASH,
                    name,
                    fakeOverrideStableName.symbol,
                    clashed.map { it.symbol },
                    context
                )
                break
            }

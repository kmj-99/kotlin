...
fun List<JsIrProgramFragment>.asTestFunctionContainers(): List<JsTestFunctionTransformer.TestFunctionContainer> {
    return filter { it.testFunction != null }
        .map {
            JsTestFunctionTransformer.TestFunctionContainer(
                it.packageFqn,
                it.nameBindings.getTestFunctionBySignature(it.testFunction),
                it.nameBindings.getSuiteFunctionBySignature(it.suiteFunction)
            )
        }
}
